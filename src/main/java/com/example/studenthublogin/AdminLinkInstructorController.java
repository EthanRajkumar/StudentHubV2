...
public class AdminLinkInstructorController extends SceneController {
    ...
    protected void onLink() {
        String instructorInput = instructorID.getText();
        ...
        ResultSet rs = SqlExecuter.RunQuery("", "SELECT * FROM INSTRUCTOR WHERE ID = " + instructorInput);

        try {
            if (!rs.next())
            {
                resultMsg.setText("Invalid instructor ID. Please try again.");
                return;
            }
        ...
        Admin.LinkInstructorToCourse(instructorInput, crnInput);
        resultMsg.setText("Instructor successfully linked to course.");
        courseTable.setItems(FXCollections.observableList(User.GetAllCourses()));
    }
}
